...
動機
----
将来的に OS に組み込み可能な学習・推論系を構築し、CPU の利点
（整数演算・分岐・キャッシュ）を最大限活かすため、
コンパイラ工学的な構成を採用する。

1) 新規性の言語化（研究上の位置づけ）
- 既存研究と重なる部分／重ならない部分を明示する  
- 研究貢献を「統一アーキテクチャ」と「離散推論設計」に限定して書く

2) 画像→構文の最小パイプライン設計
- lexer（トークン化）: どの特徴を “字句” として使うか  
- parser（構文木/AOG）: どの構文規則を用意するか  
- semantic（述語）: どの述語がクラスと結びつくか  
- classifier（推論）: SAT / MIP / ルールのどれを使うか

3) 設計の可算性（CPUで回せる範囲の定義）
- トークン数・構文規則数・探索空間の上限  
- パス探索（最適化列）の探索戦略（ビーム／焼きなまし）

4) 実験の最小構成
- MNIST / Fashion-MNIST で  
  lexer → parser → semantic → discrete classifier  
  の「一連の動作」を示す
- 既存の画像分類精度と比較するのではなく、説明性・構造性・推論可視性を指標にする

5) 失敗の定義（検証可能な形）
- どこが破綻するか（lexer / parser / semantic の失敗点）  
- 失敗時の最小修正戦略
