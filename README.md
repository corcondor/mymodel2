...
本プロジェクトの核心は**特徴設計**であり、分類器はその検証手段です。3つの分類器を用意した理由と哲学を説明します：

#### 1. 平均化クリップ型パーセプトロン（原点・純粋主義）

```
位置づけ: 離散数学の純粋な具現化
```

- **完全な整数演算**: 浮動小数点を一切使わない
- **理論的透明性**: 各重み更新の意味が明確
- **mymodel1からの継承**: MNIST実装で確立した手法

パーセプトロンは「特徴設計の品質を直接評価する」ための道具です。特徴量が良ければ線形分離でも高精度が出る——これが設計の検証基準となります。
...
- **クラス条件付き正規分布**: 各クラスの特徴分布を明示的にモデル化
- **Fisher判別**: クラス間分散 / クラス内分散の最大化
- **対角近似**: 特徴間独立性を仮定し、計算効率化

LDAは「特徴量がどのような統計的パターンを持つか」を可視化する診断ツールとして機能します。

#### 3. LightGBM（性能評価・現実的限界への挑戦）

```
位置づけ: 設計した特徴量の潜在能力を最大限引き出す
```

**採用のきっかけ**: パーセプトロンで約70%、LDAで約68%という結果に対し、「この特徴量には本当にこれ以上の情報がないのか？」という疑問が生じました。

**LightGBMを選んだ理由**:

1. **非線形境界の学習**: 離散的・ヒストグラム的な特徴量との相性が良い
2. **特徴量間の相互作用**: 決定木が自動的にAND/OR的結合を発見
3. **過学習への耐性**: 正則化とアンサンブル効果
4. **CPU効率**: GPUなしでも高速に訓練可能
5. **解釈可能性**: 重要度分析でどの特徴が効いているか可視化可能

**哲学的意義**: LightGBMは「ニューラルネット以外で、古典機械学習の限界はどこにあるか」を探る実験です。76-77%という結果は、手設計特徴量でも十分な情報がCIFAR-10画像から抽出可能であることを示しています。

#### 分類器選択のガイドライン

| 目的 | 推奨分類器 |
|------|-----------|
| 理論的純粋性・整数演算の検証 | パーセプトロン |
| 特徴空間の統計的理解 | 対角LDA |
| 最高精度の追求・特徴量評価 | LightGBM |
| 論文・発表用の比較実験 | 3つすべて |
...
```
mymodel1 (MNIST)
    ↓ 成功: 96.36% @ 手設計特徴 + パーセプトロン
    ↓ 疑問: カラー自然画像でも同じアプローチは通用するか？
    
mymodel2 (CIFAR-10) 
    ↓ Step 1: 二値マスク拡張（グレー閾値、エッジ、勾配）
    ↓ Step 2: カラー特徴追加（opponent色空間、彩度）
    ↓ Step 3: テクスチャ特徴（LBP）導入
    ↓ Step 4: 対称性（D4群）の明示的組み込み
    ↓ Step 5: 動的特徴（セルオートマトン）の実験
    ↓ Step 6: LightGBMで潜在能力を評価
    
現在: 76-77% @ 手設計特徴 + LightGBM（CPU only）
```

各ステップで「なぜこの特徴が必要か」「数学的にどう正当化されるか」を常に問いながら発展させてきました。
